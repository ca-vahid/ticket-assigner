...
export class FreshserviceService {
  ...
  /**
   * Get ticket fields including custom dropdown values
   */
  async getTicketFields(): Promise<any[]> {
    try {
      // Try the ticket fields endpoint to get form field definitions
      const response = await this.apiClient.get('/ticket_form_fields');
      return response.data.ticket_fields || response.data.fields || [];
    } catch (error) {
      // Try alternate endpoint
      try {
        const response = await this.apiClient.get('/admin/ticket_fields');
        return response.data.ticket_fields || [];
      } catch (altError) {
        this.logger.error('Failed to fetch ticket fields from Freshservice', altError);
        return [];
      }
    }
  }
...
_This recommendation was generated by the Ticket Assignment System_
  ...
  async getAgentTickets(
    ...
    try {
      ...
      this.logger.log(`Fetching tickets for agent ${agentId} with limit ${limit}`);
      
      // Simple approach: just fetch tickets without any fancy parameters
      const allTickets: FreshserviceTicket[] = [];
      let page = 1;
      const maxPages = 150; // Fetch up to 150 pages (4500 tickets) to find more agent tickets
      let totalTicketsFetched = 0;
      let consecutiveEmptyPages = 0;
      
      while (page <= maxPages && allTickets.length < limit) {
        try {
          // Very simple request - NO parameters that might cause 400 errors
          if (page % 10 === 1) {
            this.logger.debug(`Fetching pages ${page}-${Math.min(page + 9, maxPages)}...`);
          }
          
          const response = await this.apiClient.get(`/tickets?page=${page}`);
          
          const tickets = response.data.tickets || [];
          totalTicketsFetched += tickets.length;
          
          if (tickets.length === 0) {
            this.logger.debug(`No more tickets at page ${page}`);
            break;
          }
          
          // Log first ticket structure on page 1
          if (page === 1 && tickets.length > 0) {
            this.logger.debug(`First ticket structure:`, JSON.stringify(tickets[0], null, 2).substring(0, 500));
          }
          
          // Filter for this agent's resolved/closed tickets
          const agentTickets = tickets.filter(ticket => {
            const matchesAgent = ticket.responder_id === parseInt(agentId);
            const matchesStatus = statusFilter.includes(ticket.status);
            return matchesAgent && matchesStatus;
          });
          
          if (agentTickets.length > 0) {
            this.logger.debug(`Page ${page}: Found ${agentTickets.length} tickets for agent out of ${tickets.length} total`);
            consecutiveEmptyPages = 0; // Reset counter
            
            // For each agent ticket, fetch full details to get custom_fields if needed
            for (const ticket of agentTickets) {
              // If ticket already has custom_fields, use it directly
              if (ticket.custom_fields) {
                allTickets.push(ticket);
              } else {
                // Otherwise fetch full details
                try {
                  const fullTicket = await this.getTicket(ticket.id.toString());
                  allTickets.push(fullTicket);
                } catch (e) {
                  this.logger.warn(`Failed to fetch full details for ticket ${ticket.id}`);
                  allTickets.push(ticket); // Use partial ticket if full fetch fails
                }
              }
            }
          } else {
            consecutiveEmptyPages++;
            // Stop if we've had 20 pages in a row with no agent tickets
            if (consecutiveEmptyPages >= 20) {
              this.logger.debug(`Stopping after ${consecutiveEmptyPages} consecutive pages with no agent tickets`);
              break;
            }
          }
          ...
          // Stop if less than 30 tickets (default page size)
          if (tickets.length < 30) {
            this.logger.debug(`Last page reached`);
            break;
          }
      ...
      const finalTickets = allTickets.slice(0, limit);
      
      this.logger.log(`Fetched ${finalTickets.length} tickets for agent ${agentId} (searched ${totalTicketsFetched} total tickets across ${page - 1} pages)`);
